...
SupersurfelFusionRGBDBenchmarkNode::SupersurfelFusionRGBDBenchmarkNode()
    : privateNh("~"),
      it(nh),
      cpt(0),
      stop(true)
{
    ...
    privateNh.param("cam_frame_id", camFrameId, std::string("cam"));

    privateNh.getParam("trajectory_file_path", trajectoryFilePath);
    privateNh.getParam("dataset_path", datasetPath);
    ...
    gtPathPub = nh.advertise<nav_msgs::Path>("grountruth_trajectory", 1);
    ...
    privateNh.getParam("fx", cam_param.fx);
    privateNh.getParam("fy", cam_param.fy);
    privateNh.getParam("cx", cam_param.cx);
    privateNh.getParam("cy", cam_param.cy);
    privateNh.getParam("w", cam_param.width);
    privateNh.getParam("h", cam_param.height);
...
void SupersurfelFusionRGBDBenchmarkNode::publishModelMarker()
{
    ...
    model_marker_msg.header.stamp = ros::Time::now();
...
void SupersurfelFusionRGBDBenchmarkNode::publishFrameMarker()
{
    ...
    frame_marker_msg.header.stamp = ros::Time::now();
    frame_marker_msg.header.frame_id = camFrameId;
...
void SupersurfelFusionRGBDBenchmarkNode::publishDefGraph()
{
    ...
    edges_msg.header.stamp = ros::Time::now();
...
void SupersurfelFusionRGBDBenchmarkNode::publishDefConstraints()
{
...
void SupersurfelFusionRGBDBenchmarkNode::publishLocalMapPoints()
{
    ...
    pcl_conversions::toPCL(ros::Time::now(), local_map_cloud->header.stamp);
    localMapPub.publish(local_map_cloud);
}

void SupersurfelFusionRGBDBenchmarkNode::configCallback(supersurfel_fusion::SupersurfelFusionRGBDBenchmarkConfig &config, uint32_t level)
{
    stop = config.stop;
}

void SupersurfelFusionRGBDBenchmarkNode::run()
{
    std::ifstream ifs;
    ifs.open(datasetPath + "associations_with_gt.txt");
    std::ofstream trajectory_file;
    trajectory_file.open(trajectoryFilePath + "estimated.txt", std::ofstream::out);

    if(!ifs)
        std::cout<<"Unable to open file: "<<datasetPath<<std::endl;
    else
    {
        while(ifs.good() && ros::ok())
        {
            if(!stop)
            {
                ...
                rgb_path = datasetPath + "/" + words[1];
                depth_path =  datasetPath + "/" + words[3];

                if(cpt == 0)
                    firstGtPose = tf::Transform(tf::Quaternion(std::stof(words[8]), std::stof(words[9]), std::stof(words[10]), std::stof(words[11])),
                                                tf::Vector3(std::stof(words[5]), std::stof(words[6]), std::stof(words[7])));

                tf::Transform gt_pose(tf::Quaternion(std::stof(words[8]), std::stof(words[9]), std::stof(words[10]), std::stof(words[11])),
                                      tf::Vector3(std::stof(words[5]), std::stof(words[6]), std::stof(words[7])));

                cv::Mat bgr, rgb, depth, depth_u16;
                bgr = cv::imread(rgb_path);
                cv::cvtColor(bgr, rgb, CV_BGR2RGB);
                depth_u16 = cv::imread(depth_path, cv::IMREAD_ANYDEPTH);
                depth_u16.convertTo(depth, CV_32FC1, depthScale);
                ...
                if(rgb.data != NULL && depth.data != NULL)
                {
                    ...
                    tf::StampedTransform stamped_opt_to_map(opt_to_map, ros::Time::now(), mapFrameId, camFrameId);
                    ...
                    gtPath.header.frame_id = mapFrameId;
                    geometry_msgs::PoseStamped gt_pose_stamped_msg;
                    tf::poseStampedTFToMsg(tf::Stamped<tf::Transform>(firstGtPose.inverse() * gt_pose, ros::Time::now(), mapFrameId), gt_pose_stamped_msg);
                    gtPath.poses.push_back(gt_pose_stamped_msg);

                    nav_msgs::Odometry odom_msg;
                    odom_msg.header.stamp = ros::Time::now();
                    odom_msg.header.frame_id = mapFrameId;
                    odom_msg.child_frame_id = camFrameId;
                    tf::poseTFToMsg (opt_to_map, odom_msg.pose.pose);

                    if(ssf.getStamp() > 1)
                    {
                        tf::Transform delta_base = prevOptToMap.inverse() * opt_to_map;
                        double delta_t = (ros::Time::now() - prevTime).toSec();
                    ...
                    if(gtPathPub.getNumSubscribers() > 0)
                    ...
                    if(constraintsPub.getNumSubscribers() > 0)
                        publishDefConstraints();

                    if(localMapPub.getNumSubscribers() > 0)
                        publishLocalMapPoints();
                    ...
                    if(display)
                    {
                        ...
                        cv::imshow("slanted planes", im_slanted_plane * 0.2f);

                        cv::waitKey(1);
                    }

                    trajectory_file<<words[0]<<" "<<
                                     opt_to_map.getOrigin().x()<<" "<<opt_to_map.getOrigin().y()<<" "<<opt_to_map.getOrigin().z()<<" "<<
                                     opt_to_map.getRotation().x()<<" "<<opt_to_map.getRotation().y()<<" "<<opt_to_map.getRotation().z()<<" "<<opt_to_map.getRotation().w()<<std::endl;
                }
